...
{
   # Display Help
   echo "KServe quick uninstall script."
   echo
   echo "Syntax: [-s|-r]"
   echo "options:"
...
then
    ...
    bin/istioctl manifest delete -f istio-minimal-operator.yaml -y;
fi
...
kubectl delete --validate=false -f https://github.com/jetstack/cert-manager/releases/download/${CERT_MANAGER_VERSION}/cert-manager.yaml
...
for i in 1 2 3 4 5 ; do kubectl delete -f https://github.com/kserve/kserve/releases/download/${KSERVE_VERSION}/${KSERVE_CONFIG} && break || sleep 15; done
# Clean up
