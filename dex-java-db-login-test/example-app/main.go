...
func cmd() *cobra.Command {
	...
	c := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("TEST3")
			http.HandleFunc("/fastapi", a.handleFastapi)
...
///SUJUNE
func (a *app) handleFastapi(w http.ResponseWriter, r *http.Request) {
	fmt.Println(" Handle FastAPI")
	var scopes []string
	...
	fmt.Println("[SDT]", authCodeURL)
	if connectorID != "" {
		authCodeURL = authCodeURL + "&connector_id=" + connectorID
	}
	authCodeURL = "http://192.168.1.207:30487"
...
func (a *app) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("[SDT]", reflect.TypeOf(authCodeURL))
	fmt.Println("[SDT]", scopes)
	fmt.Println("[SDT]", authCodeURL)
	fmt.Println("[SDT]", http.StatusSeeOther)
